...
const db = require(path.join(__dirname+"/../../../banco"));
...
async function algo(email,token,metodo) {
  console.log("comecou");
  var conta;
  if (metodo == 0)
      var query = "Select * from Usuarios u inner join Email e on u.CodUsuario=e.CodUsuario where Email=? AND Fim is null AND Senha=?";
   else
     var query = "Select * from Email inner join Transportes on CodUsuario=CodGerente where Email=? AND Matricula=?";
  let values = [email,token];
  results = await db.pegaDados(query, values);
  if (results == "")
      console.log("Opah")
   else {
     iniciarUsuario(results,metodo);
   }
}

async function insert(tabela,dados) {
...
module.exports = {algo,usuario,transportes,preferencias,done,link};
